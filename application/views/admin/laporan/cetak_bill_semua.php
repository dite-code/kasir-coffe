...
$mpdf = new \Mpdf\Mpdf([
		...
		?>
		<table border="0" width="100%">
		<tr><td style="text-align:center" colspan="6">Pesanan Dapur</td></tr>
		<tr><td style="text-align:center" colspan="6"><?= $tanggal. " " . $jam; ?></td></tr>
		<tr><td colspan="6">No Meja : <?= $row_ord->no_meja; ?></td></tr>
		<tr><td colspan="6">---------------------------------------------------------</td></tr>
		<tr>
		...
		$no = 1;
        $ttl_pemb = 0;
        foreach ($get_transOrderDetail_dapur as $row) {
			$sub_ttl = $row->harga_menu * $row->qty;
            $ttl_pemb += $sub_ttl;
            ?>
			<tr>
		...
		?>
		<table border="0" width="100%">
		<tr><td style="text-align:center" colspan="6">Pesanan BAR</td></tr>
		<tr><td style="text-align:center" colspan="6"><?= $tanggal. " " . $jam; ?></td></tr>
		<tr><td colspan="6">No Meja : <?= $row_ord->no_meja; ?></td></tr>
		<tr><td colspan="6">---------------------------------------------------------</td></tr>
		<tr>
		...
		$no = 1;
        $ttl_pemb = 0;
        foreach ($get_transOrderDetail_bar as $row) {
			$sub_ttl = $row->harga_menu * $row->qty;
            $ttl_pemb += $sub_ttl;
            ?>
			<tr>

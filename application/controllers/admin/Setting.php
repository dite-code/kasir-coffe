...
class Setting extends Temp {
    ...
    public function edit_profil() {
        if ($this->session->userdata('is_login')) {
            ...
            $data['name_page'] = 'Setting';
            $data['name_page_small'] = 'Setting Profil Admin';
            $record['row_us'] = $this->Model_setting->get_setUserWhereId($id);
            $data['content'] = $this->load->view('admin/setting/edit_profil', $record, TRUE);
            if ($a['level_user'] == 1) {
            $this->load->view('temp_admin/layout', $data);
            }else{
    ...
    public function set_user() {
        if ($this->session->userdata('is_login')) {
            ...
            if ($a['level_user'] == 1) {
                $data = $this->layout_admin();
                $data['name_page'] = 'Setting';
                $data['name_page_small'] = 'Setting User';
                $record['get_setUser'] = $this->Model_setting->get_setUser();
                $record['jml_lvl_user_1'] = $this->Model_setting->count_lvl_user_1('1');
                $record['get_refLevelUser'] = $this->Model_ref->get_refLevelUser();
                $record['javasc'] = $this->load->view('admin/js', NULL, TRUE);
                $data['content'] = $this->load->view('admin/setting/set_user', $record, TRUE);
                $this->load->view('temp_admin/layout', $data);
            } else {
                redirect('login');
            }
    ...
    function crop_ImgUpload() {
        ...
        $imageName = date('dmYHis') . '.png';
        $cont = file_put_contents($imageName, $data);
        rename($imageName, 'assets/img/tampungan/' . $imageName);
        echo '<img src="' . base_url('assets/img/tampungan/') . $imageName . '" class="img-thumbnail" />';
        echo '<input type="hidden" name="foto" value="' . $imageName . '">';
    }
    ...
    function update_profil() {
        ...
        if ($imageName != '') {
            rename('assets/img/tampungan/' . $imageName, 'assets/img/' . $imageName);
            $data['logo_cofe'] = $imageName;
            $path = '././assets/img/';
            unlink($path . $img);
        }
        $data['alamat'] = $alamat;
        $data['ppn'] = $ppn;
        $data['catatan_kaki'] = $catatan_kaki;
        $data['nama'] = $nama;
    ...
    function update_profilAdmin() {
        ...
        $imageName = $this->input->post('foto');
        if ($imageName != '') {
            rename('assets/img/tampungan/' . $imageName, 'assets/img/user/' . $imageName);
            $data['foto'] = $imageName;
        ...
        $data['no_telpon'] = $no_telpon;
        $data['email'] = $email;
        $query = $this->Model_aksi->update('id', $id, 'user', $data);
        if ($query) {
            $this->session->set_flashdata('tipe', 'alert-success');
            $this->session->set_flashdata('msg', 'Berhasil Disimpan');
    ...
    function updateKunci() {
        ...
        $qry = $this->Model_aksi->update('id', $id, 'user', $data);
        if ($qry) {
            echo 'true';
        } else {
            echo 'false';
    ...
    function deleteUser() {
        $id = $this->input->post('id');
        $qry = $this->Model_aksi->delete('id', $id, 'user');
        if ($qry) {
            echo 'true';
        } else {
            echo 'false';
